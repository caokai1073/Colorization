...
class Feature_colorization(object):
	def __init__(self, 
		...
		self.f_f0 = batch_norm(name='f_f0')
		self.f_f1 = batch_norm(name='f_f1')
		self.f_f2 = batch_norm(name='f_f2')
		self.f_q1 = batch_norm(name='f_f3')
		self.f_q0 = batch_norm(name='f_f4')
		'''
		self.f_h0 = batch_norm(name='f_h0')
		self.f_h1 = batch_norm(name='f_h1')
		self.f_h2 = batch_norm(name='f_h2')
		self.f_h3 = batch_norm(name='f_h3')
		self.f_h4 = batch_norm(name='f_h4')
		'''
	...
	def Feature_G2C(self, feature_G, stddev=0.02):
		with tf.variable_scope("f_G2C") as scope:	
			#s = feature_G.get_shape()[1]
			s, dim = int(np.shape(feature_G)[1]), int(np.shape(feature_G)[3])
			feature = tf.reshape(feature_G, [self.batch_size, s*s*dim])
			'''
			w1 = tf.get_variable('f_w1', [s*s*dim, s*s*dim*2],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
			b1 = tf.get_variable('f_b1', [s*s*dim*2], initializer=tf.constant_initializer(0.0))
			h1 = lrelu(self.f_h1(tf.matmul(feature,w1) + b1))

			w2 = tf.get_variable('f_w2', [s*s*dim*2, s*s*dim*4],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
			b2 = tf.get_variable('f_b2', [s*s*dim*4], initializer=tf.constant_initializer(0.0))
			h2 = lrelu(self.f_h2(tf.matmul(h1,w2) + b2))
			
			w3 = tf.get_variable('f_w3', [s*s*dim*4, s*s*dim*2],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
			b3 = tf.get_variable('f_b3', [s*s*dim*2], initializer=tf.constant_initializer(0.0))
			h3 = lrelu(self.f_h3(tf.matmul(h2,w3) + b3))

			w4 = tf.get_variable('f_w4', [s*s*dim*2, s*s*dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
			b4 = tf.get_variable('f_b4', [s*s*dim], initializer=tf.constant_initializer(0.0))
			h4 = lrelu(self.f_h4(tf.matmul(h3,w4) + b4))
			
			q0 = tf.reshape(h4, [self.batch_size, s, s, dim])
			'''	
			
			
			s2 = int(s/2)

			f0 = lrelu(self.f_f0(conv2d(feature_G, dim*4, k_h=1, k_w=1, d_h=1, d_w=1, name='f_conv0')))
			# f0 is (2 x 2 x 2048)
			f1 = lrelu(self.f_f1(conv2d(f0, dim*8, k_h=2, k_w=2, name='f_conv1')))
			# f1 is (1 x 1 x 4096)
			#f2 = lrelu(self.f_f2(conv2d(f1, dim*4, name='f_conv2')))

			#q2 = lrelu(self.f_q2(deconv2d(f2, [self.batch_size, s4, s4, dim*2], name='f_deconv2')))
			q1 = lrelu(self.f_q1(deconv2d(f1, [self.batch_size, s, s, dim*4], k_h=2, k_w=2, name='f_deconv1')))
			# q1 is (2 x 2 x 2048)
			q0 = tf.nn.sigmoid(self.f_q0(deconv2d(q1, [self.batch_size, s, s, dim], k_h=1, k_w=1, d_h=1, d_w=1, name='f_deconv0')))
			# q0 is (2 x 2 x 512)
	...
	def Autoencoder(self, image, reuse=False):
		...
		with tf.variable_scope("Autoencoder") as scope:
			...
			# e6 is (4 x 4 x 512)
			e7 = tf.nn.sigmoid(conv2d(e6, self.dim*8, name='e_conv7'))
			# e7 is (2 x 2 x 512)
	...
	def Autodecoder(self, feature, e6, e5, e4, e3, e2, e1, e0, reuse=False):
		...
		with tf.variable_scope("Autodecoder") as scope:
			...
			output = (deconv2d(d0, [self.batch_size, s, s, self.output_c_dim], d_h=1, d_w=1, name='d_output'))
	...
	def train(self):
		...
		with tf.device("/cpu:0"):
				...
				for i in range(self.num_gpus):
					with tf.device('/gpu:%d' % i):
						with tf.name_scope('%s_%d' % ('GPU', i)) as scope:		
							...
							self.sparse_p = tf.reduce_mean(self.feature_C)
							
							self.sparse_cost = tf.reduce_sum(self.sparse_index*tf.log(self.sparse_index/self.sparse_p)+(1-self.sparse_index)*tf.log((1-self.sparse_index)/(1-self.sparse_p)))
							self.encoder_loss = 0.5 * tf.reduce_mean(tf.pow(tf.subtract(self.image_G_, self.image_C_batch),2.0))+ 0.5 * tf.reduce_mean(tf.pow(tf.subtract(self.image_C_,self.image_C_batch),2.0)) + 10 * self.sparse_cost
			...
			for epo in xrange(self.epoch):
				...
				data_C.sort()
				data = list(zip(data_G,data_C))
				np.random.shuffle(data)
				data_G = [x[0] for x in data]
				data_C = [x[1] for x in data]
				...
				for idx in xrange(batch_idxs):
					...
					if self.train_type == 'e_loss':
						...
						_ , encoder_loss, feature_loss = self.sess.run([apply_gradient_op, self.encoder_loss, self.feature_loss] , feed_dict={self.image_G: batch_images_G, self.image_C: batch_images_C})
					...
					if self.train_type == 'e_loss':
						...
						print("Epoch: [%2d] [%4d/%4d] time: %4.4f, encoder_loss: %.8f, feature_loss: %.8f" \
						% (epo, idx+1, batch_idxs,
							time.time() - start_time, encoder_loss, feature_loss))
	...
	def test(self):
		...
		for idx in range(idxs):
			...
			decoder_imgs_G, decoder_imgs_C, encoder_loss, feature_loss, feature_G, feature_C, feature_G_ = self.sess.run([self.image_G_, self.image_C_, self.encoder_loss, self.feature_loss, self.feature_G, self.feature_C, self.feature_G_], feed_dict={self.image_G: batch_images_G, self.image_C: batch_images_C})
